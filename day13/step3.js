/*
    p.174 반복문
        ...
        - 형태
            ...
            9. [3실행문] 실행코드
            10.[4증감식] 증감식
                // 도대체 언제까지 1234가 반복되는지?? 2조건문에 false될때까지
            *1234 -> 234 -> 234
...
for(let i = todos.length-1; i>=0; i--){
   // - i는 마지막 인덱스 부터 0까지 1씩감소 하면서 console.log 반복실행
   console.log(`${i}번째 할 일 : ${todos[i]}`);
}

let outPrint= ``; //공백이 저장된 변수
// 예2) 1부터 10이하까지 1씩 증가 반복[한줄표시]; console.log() 하나당 줄바꿈 자동.
for(let i =1; i <=10; i++){
    outPrint += ` ${i} `;
} // for end
...
sum = 0;
for(let i= 1; i<=100; i++){
    if(i %7 ==0) {sum += i;} // 만약에 i가 나누기 7을 했을 때 나머지가 0이면 i는 7배수
} // for end

// for 중첩 [상위 for문 1회전마다 ]
for(let 부모 = 1; 부모<=5; 부모++){
    // - 부모는 1부터 5이하까지 1씩증가 반복 [총 5회전]
    console.log(`====${부모}====`)
    for(let 자식 = 1; 자식<=3; 자식++){
        // - 자식은 1부터 3이하까지 1씩 증가 반복 [총 3회전 => x 상위for => 15회전]
...
for(let gob = 1; gob<=9; gob++){console.log(`곱:${gob}`)} 
        ...
        // 3. 단 과 곱 합치기 [단 안에 곱이 들어간다. 단 for 상위 , 곱for 하위]

for(let dan = 2; dan<=9; dan++){
    for (let gob =1; gob<=9; dan++){
