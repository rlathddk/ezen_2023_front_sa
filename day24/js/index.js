...
- 댓글(하위)
    1. 관계파악
    2. 데이터
        게시물 : 대표이지?, 등록날짜, 내용, 좋아요수, 싫어요수, 비밀번호(삭제/수정시 사용)
...
// [1]모든 피드(게시물) 출력함수 (1.js열렸을 때 2.등록, 삭제, 댓글등록/삭제)
function 모든피드호출(){
    ...
    let html = '안녕';
        ...
        for(let i = 0; i<feedList.length; i++){
            ...
            html += `
            <div class="feed">
            <div><img src="img/${feed.fimg}" alt=""></div>
...
function 댓글출력(fno){ // 매개변수 : 출력한 게시물번호
    let html = `안녕`

    for(let i = 0; i<replyList; i++){
        if(fno == replyList[i].fno){ // 댓글목록에서 동일한 fno 찾기
            html += `
                <div class="reply">
                    <div>꼭 한번 놀러가보고 싶네요.</div>
                    <div>
                        <span class="date">30분전</span>
...
function 수정페이지로이동(fno){ // 매개변수 : 수정할 pno
    localStorage.setItem('updatefno', JSON.stringify(fno))
    location.href ="update.html"

}
// [5] 패스워드 검사 함수 (1. 삭제/수정 했을 때)
function 패스워드확인(fno) {
      // 1. 확인패스워드 입력
